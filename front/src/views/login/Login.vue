    ...
    <div class="login-wrapper">
        <div class="login-cart">
            ...
            <div class="input-wrapper">
                ...
                <input placeholder="请输入账号" v-model="logindata.username" />
            </div>
            <div class="input-wrapper">
...
async function loginHandle() {
    ...
    const result = await reLogin(formData);
    if (result.status != 200 || result.data.code == 400) {
        document.title = '';
        alert("用户名或密码错误，请重试")
    }
    else {
        userStore.updataToken("Bearer " + result.data.token);
