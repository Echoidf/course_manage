<template>
    <a class="linkto" v-if="!item.children || item.children.length === 0" @click="routeTo(item)"
        :class="{ active: item.name == route.name }" @mouseenter="changeTip(item, $event)"
    ...
    <template v-else>
        <div class="menu-title" @click="menuclick" @mouseenter="changeTip(item, $event)"
            @mouseleave="changeTip(false, $event)">
            <span class="tab" v-for="i in tab"></span>
        ...
        <div class="submenu">
            <!-- 递归调用以支持多级路由菜单 -->
            <ItemMenu v-for="subitem in item.children" :item="subitem" :tab="tab + 1" :menuHover="menuHover"
                :changeMenuTip="changeMenuTip" :key="item.path + '-' + item.name" />
        </div>
    </template>
</template>
